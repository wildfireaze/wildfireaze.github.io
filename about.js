document.addEventListener("DOMContentLoaded", function () {
  ...
  // Bind the click event to the button instead of using inline onclick
  ...
  function updateContent() {
    ...
    if (datasetSection) {
      ...
      // Update the Example Images text (assumes it's in a container with class .image-text)
      const exampleImagesText = document.querySelector(".image-text p");
      if (exampleImagesText) {
        exampleImagesText.textContent = i18next.t("aboutPage.dataset.exampleImages");
      }
      // Update the Download Info text (assumes it's the last paragraph in the section)
      const downloadInfoText = datasetSection.querySelector("p:last-of-type");
      if (downloadInfoText) {
        downloadInfoText.textContent = i18next.t("aboutPage.dataset.downloadInfo");
      }
    }
    ...
    if (projectSection) {
      ...
      // Update the first paragraph (general project description)
      const projectPara = projectSection.querySelector("p");
      if (projectPara) {
        projectPara.textContent = i18next.t("aboutPage.project.p");
      }
      // Update the second paragraph (detailed project info)
      const projectInfoText = projectSection.querySelector("p:last-of-type");
      if (projectInfoText) {
        projectInfoText.textContent = i18next.t("aboutPage.project.projectInfo");
      }
    }
